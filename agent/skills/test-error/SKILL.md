...
name: test-error
description: CIエラーの根本原因調査・解決方針検討
tier: "1"
runtime: Claude Code / Codex
---

CIエラーが発生しています。CIログをチェックして下さい。
...
| 3b. コンテキスト解決 | `test-error → research`（確定）: ドメイン research で固定 |
...
Step 0 b' で解決されたドメインのペルソナ（`persona_refs × phase_affinity` で各フェーズに自動割当）がディスカッションして根本原因と解決方針を検討します。

### Phase: plan

CIエラーの内容を整理し、調査計画を策定する。エラーログの分析対象範囲、関連するコード・DR・仕様書、確認すべき観点を明確にする。

### Phase: implement

CIログ、DR、コード、仕様書を確認し、ディスカッションを通じて、根本的解決の方針を検討してください。
報告は日本語でお願いします。
...
原因分析・方針検討後、以下を提案すること：

- **コード修正が必要な場合**: `/fix` で課題修正
- **大規模な修正が必要な場合**: `/task` で作業計画を起票
- **設定・環境の問題の場合**: 具体的な修正手順を提示
- **仕様起因の場合**: `/spec-update` でDR・仕様書を更新
